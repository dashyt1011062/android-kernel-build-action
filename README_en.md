...
`ghcr` maps to `ghcr.io/feicong/droid-ddk`, while `dockerhub` maps to `docker.io/fsx199/droid-ddk`. GitHub Actions uses `ghcr` by default; callers can select `dockerhub` for their network environment. The action writes the target `version`, `mode=docker`, and the selected `source` to `.dddk-config` in the current workspace; it does not write `$HOME/.droid-ddk/source` or `$HOME/.droid-ddk/mode`. The action omits `--platform`, so `dddk` selects the runner host architecture.
